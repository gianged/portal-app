...
/// Opaque handle for one spooled entry, returned by [`Spool::peek`] and passed
/// back to [`Spool::ack`]. The adapter assigns and interprets it.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct SpoolId(pub u64);

/// One peeked backlog entry: its id plus the opaque payload bytes that were
...
/// drainer peeks, replays, and acks once the backend recovers.
///
...
pub trait Spool: Send + Sync {
    ...
    /// Peek up to `max` entries from the head without removing them. Peeking
    /// again before an ack returns the same head entries.
    ...
    /// contiguous head prefix of the last [`Spool::peek`].
