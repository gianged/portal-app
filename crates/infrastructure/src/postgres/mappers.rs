...
pub(crate) fn month_bounds(year: i32, month: u8) -> Result<(Date, Date), RepositoryError> {
    let m = Month::try_from(month).map_err(|_| RepositoryError::Backend("invalid month".into()))?;
