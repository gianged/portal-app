use std::{fmt::Display, time::Duration};

use async_trait::async_trait;
use redis::{AsyncCommands, aio::ConnectionManager};

use domain::{error::PresenceError, ids::UserId, ports::presence::Presence};

use super::connect;

/// `SETEX`-backed presence store; every key carries a TTL so a crashed process stops showing online.
#[derive(Clone)]
pub struct PresenceStore {
    conn: ConnectionManager,
}

impl PresenceStore {
    pub async fn new(url: &str) -> Result<Self, PresenceError> {
        let conn = connect::connect_manager(url).await.map_err(backend)?;
        Ok(Self { conn })
    }
}

#[async_trait]
impl Presence for PresenceStore {
    /// Mark the user online for `ttl`; call again before expiry to heartbeat.
    #[tracing::instrument(skip_all, fields(user = ?user, ttl = ?ttl))]
    async fn set_online(&self, user: UserId, ttl: Duration) -> Result<(), PresenceError> {
        let key = presence_key(user);
        let mut conn = self.conn.clone();
        conn.set_ex::<_, _, ()>(key, 1u8, ttl.as_secs())
            .await
            .map_err(backend)?;
        Ok(())
    }

    #[tracing::instrument(skip_all, fields(user = ?user))]
    async fn is_online(&self, user: UserId) -> Result<bool, PresenceError> {
        let key = presence_key(user);
        let mut conn = self.conn.clone();
        conn.exists::<_, bool>(key).await.map_err(backend)
    }
}

fn presence_key(user: UserId) -> String {
    format!("portal:presence:user:{}", user.0)
}

fn backend<E: Display>(e: E) -> PresenceError {
    PresenceError::Backend(e.to_string())
}
