...
pub fn policy_dto(p: &AttendancePolicy) -> PolicyDto {
    PolicyDto {
        workday_start: super::fmt_time(p.workday_start),
        work_hours_per_day: p.work_hours_per_day,
        flex_core_start: super::fmt_time(p.flex_core_start),
        flex_core_end: super::fmt_time(p.flex_core_end),
        flex_daily_min: p.flex_daily_min,
        flex_daily_max: p.flex_daily_max,
        flex_earliest_start: super::fmt_time(p.flex_earliest_start),
        flex_latest_end: super::fmt_time(p.flex_latest_end),
...
pub fn update_policy_command(
    req: &UpdatePolicyRequest,
) -> Result<UpdatePolicyCommand, SharedError> {
    Ok(UpdatePolicyCommand {
        workday_start: super::to_time(&req.workday_start, "Workday start")?,
        work_hours_per_day: req.work_hours_per_day,
        flex_core_start: super::to_time(&req.flex_core_start, "Flex core start")?,
        flex_core_end: super::to_time(&req.flex_core_end, "Flex core end")?,
        flex_daily_min: req.flex_daily_min,
        flex_daily_max: req.flex_daily_max,
        flex_earliest_start: super::to_time(&req.flex_earliest_start, "Flex earliest start")?,
        flex_latest_end: super::to_time(&req.flex_latest_end, "Flex latest end")?,
